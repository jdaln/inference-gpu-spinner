...
Most ⬜ rows sit on a plan above `MAX_EUR_PER_HOUR` (default €10), so reaching one takes a
deliberate `--allow-expensive`. **The cheaper ones do not**: `qwen38-flash-next` and
`glm53-flash-nvfp4` are €6.60/h, under the limit, so a bare `bin/spin up --model <name>` deploys a
4-GPU box with only a debug warning about the missing validation. Those are the rows that matter
most, because an accidental user reaches them.
...
> **What prefix caching means for the numbers below (added 2026-09-22).** On a hybrid checkpoint a
> cache hit makes the model answer fast and wrong — mechanism and measurement in
> [gpu-spinner.md](gpu-spinner.md#hybrid-checkpoints-and-prefix-caching). Until 2026-09-22
> `bin/spin ceiling` and `bin/spin soak` reused one haystack across probes, so every probe after the
> first was part-served from cache; `_gs_prompt` now opens each prompt with a unique session tag.
>
> A cache hit answers wrongly, so it can never manufacture a false pass: **every PASS recorded here
> stands.** What wants re-measuring is any *failure*, any cap derived from one, and any latency
> figure on the hybrid rows — `qwen36`, `qwen36-27b-nvfp4`, `qwen36-35b`, `qwen36-35b-nvfp4`,
> `qwen38`, `qwen38-nvfp4`, `qwen38-flash-next`, `glm53-flash`, `glm53-flash-nvfp4` — since a cached
> probe skips prefill and those p50s are floors rather than cold-start timings. The
> `glm53-flash-nvfp4` 98,304 cap is outside that set: it is a crash caught by the container restart
> counter, which this defect cannot cause.
...
| `qwen38` | Qwen3.8-27B (dense hybrid) | FP8 | **L40S** (its own lane) | 131072 | ✅ | 359,719 tok | 2.74× | 41,023/46,068 MiB | 2026-09-21 | Measured on `GPU-SPOT-8xCPU-64GB-1xL40S` (€0.83/h), the lane a bare `bin/spin up --model qwen38` reaches. FLASHINFER attention; load 27.57 GiB in 72.9 s; init 51.2 s. soak 3×20 @32768: factual 6/6, traps 2/2, **60/60 needles**, 0 wrong; p50/p95 73.2 s/126.2 s. `ceiling`: 128,766 prompt_tokens passes (full deployed context). **Native 262144 also tested** (`-e max_model_len=262144`): KV 372,123, 1.42×, a single 259,512-token request passes in 173.8 s, and 3 concurrent at 260k all return correct answers with **0 rejected** — but p50 391 s, max 586 s. So 262144 is safe for correctness and unusable interactively; 131072 stays the default because it buys a 2.74× multi-user envelope at a 73 s p50. Earlier RTX PRO 6000 figures (1,743,257 tok, 13.30×) are 4.8× more KV and do not transfer |
...
| `deepseek-v4-flash-vision` | DeepSeek-V4-Flash-Vision-Exp (285B-A13B MoE, multimodal) | FP4+FP8 | **4×B200** (`…4xB200`, TP=4+EP) | 32768 | ⬜ | — | — | — | — | gated by price (€18/h, needs `--allow-expensive`); ~168 GB of weights, ~202 GB VRAM budget before KV. Needs the pinned `vllm/vllm-openai:deepseekv4-flash-vision` image — the stable wheel routes this checkpoint to the text-only class. Published vision runs exist only on GB200 NVL4 (TP4+EP) and MI350X, neither offered here 🚫 |
| `deepseek-v41-flash` | DeepSeek-V4.1-Flash (552B + 196B Engram MoE) | MXFP4+MXFP8 | **4×B200** (`…4xB200`, TP=4) | 1048576 | ⬜ | — | — | — | — | Newly added; gated by price (€18/h, needs `--allow-expensive`). ~511 GB on disk / 476 GiB, recipe floor 614 GB VRAM — one GB200 NVL4 tray at TP4, which 4×B200 matches. Needs a vLLM nightly from 2026-09-10 or later; no release serves `deepseek_v41`. Engram tables alone are 183 GiB 🚫 |
| `deepseek-v4-pro-0813` | DeepSeek-V4-Pro-0813 (1.6T-A49B MoE) | FP4+FP8 | **8×B200** (`…8xB200`, TP=8) | 393216 | ⬜ | — | — | — | — | Newly added; gated by price (€36/h, needs `--allow-expensive`). ~893 GB of weights; the recipe states it does not fit one 768 GB tray and all its recommended deployments are 8-GPU. ~€36/h and a ~1000 GB weights disk. `max_model_len` 393216 is the floor for the "max" reasoning mode 🚫 |
| `k2-horizon-375b` | K2-Horizon-375B-A23B (379B-A27B MoE) | FP8 | **lane: 8×RTX PRO 6000** (`…8xRTXPRO6000`, TP=8) | 131072 (native 524288) | ⬜ | — | — | — | — | Newly added; gated by price (€13.20/h, needs `--allow-expensive`). ~391 GB of weights. `K2HorizonForCausalLM` is in neither v0.23.0 nor v0.29.0, so it pins the same dated nightly as `deepseek-v41-flash`. The recipe's only verified run is BF16 at TP8 on MI355X; this serves the FP8 build at TP8 on 8× RTX PRO 6000 (13.20 €/h vs 18.00 for 4×B200), which is both cheaper and the same TP as the published run. 4× RTX PRO 6000 cannot hold it: 391 GB of weights against 380 GB 🚫 |
| `glm53-flash` | GLM-5.3-Flash (321B-A18B MoE, multimodal) | FP8 | **4×B200** (`…4xB200`, TP=4) | 1048576 | ⬜ | — | — | — | — | Newly added; gated by price (€18/h, needs `--allow-expensive`). ~306 GiB of weights. `Glm5NextForConditionalGeneration` is in neither v0.23.0 nor v0.29.0 — needs the pinned `vllm/vllm-openai:glm53-flash` image. FP8 KV cache is Blackwell-only for this model; Hopper must run BF16 KV. **Kept deliberately alongside the validated `glm53-flash-nvfp4`, which is not a substitute**: the NVFP4 sibling is capped at a *measured* 98304 while this profile carries the recipe's 1048576, and it needs no third-party kernel overlay. Its 1M is unmeasured. **Superseded 2026-09-22:** the prediction that the NVFP4 build's ~100k boundary applies here too no longer holds unqualified — `custom_model` (line below) reached 1,044,002 prompt tokens on the same 45-layer/34-KDA geometry, the same pinned image digest and the same overlay commit, so the vllm#54317 crash is not an unconditional property of the architecture at that length and is worth re-attributing to the NVFP4 build. That is a hypothesis; nobody has run this profile. Anyone paying 18.00 €/h for the long-context lane should still ceiling-search it first (`bin/spin ceiling`), and note this profile now pins `--no-enable-prefix-caching` (vllm#56960 is filed against this exact checkpoint) 🚫 |
...
- **2026-07-05 spot-check + `requires_review` cleared:** re-ran gemma4-26b & gemma4-31b (L40S) and
  ...
  row, not re-run). **glm52 FP8 stays gated by cost choice** (8×B200 ~36 €/h), not doubt. (The
  `requires_review` key itself was retired 2026-09-21; the plan's price gates a deploy now.)
